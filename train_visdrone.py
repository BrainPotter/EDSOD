...
# 数据集路径
# DATASET_ROOT = '/media/lijunnian/LJN-5T/DiffusionDet/data/visdrone/coco_format/'
DATASET_ROOT = '/root/autodl-tmp/DiffusionDet/datasets/visdrone/coco_format/'
ANN_ROOT = os.path.join(DATASET_ROOT, 'annotations/')

TRAIN_PATH = os.path.join(DATASET_ROOT, 'train_images/')
VAL_PATH = os.path.join(DATASET_ROOT, 'val_images/')
TEST_PATH = os.path.join(DATASET_ROOT, 'test_images/images/')

TRAIN_JSON = os.path.join(ANN_ROOT, 'train_label.json')
VAL_JSON = os.path.join(ANN_ROOT, 'val_label.json')
TEST_JSON = os.path.join(ANN_ROOT, 'test_label.json')

# 声明数据集的子集
PREDEFINED_SPLITS_DATASET = {
    "visdrone_my_train": (TRAIN_PATH, TRAIN_JSON),
    "visdrone_my_val": (VAL_PATH, VAL_JSON),
}
...
def plain_register_dataset():
    # 训练集
    DatasetCatalog.register("visdrone_my_train", lambda: load_coco_json(TRAIN_JSON, TRAIN_PATH))
    MetadataCatalog.get("visdrone_my_train").set(thing_classes=CLASS_NAMES,  # 可以选择开启，但是不能显示中文，这里需要注意，中文的话最好关闭
    ...
    # 验证/测试集
    DatasetCatalog.register("visdrone_my_val", lambda: load_coco_json(VAL_JSON, VAL_PATH))
    MetadataCatalog.get("visdrone_my_val").set(thing_classes=CLASS_NAMES,  # 可以选择开启，但是不能显示中文，这里需要注意，中文的话最好关闭
                                           evaluator_type='coco',  # 指定评估方式
                                           json_file=VAL_JSON,
                                           image_root=VAL_PATH)
...
# 可选择使用此方法
def checkout_dataset_annotation(name="visdrone_my_val"):
...
def main(args):
    cfg = setup(args)
    #TODO
    plain_register_dataset()  # here to register
    if args.eval_only:
