...
# 数据集路径
# DATASET_ROOT = '/media/lijunnian/LJN-5T/DiffusionDet/data/uavdt/'
DATASET_ROOT = '/root/autodl-tmp/DiffusionDet/datasets/uavdt/'
...
PREDEFINED_SPLITS_DATASET = {
    "uavdt_my_train": (TRAIN_PATH, TRAIN_JSON),
    "uavdt_my_val": (VAL_PATH, VAL_JSON),
}
...
def plain_register_dataset():
    # 训练集
    DatasetCatalog.register("uavdt_my_train", lambda: load_coco_json(TRAIN_JSON, TRAIN_PATH))
    MetadataCatalog.get("uavdt_my_train").set(thing_classes=CLASS_NAMES,  # 可以选择开启，但是不能显示中文，这里需要注意，中文的话最好关闭
    ...
    # 验证/测试集
    DatasetCatalog.register("uavdt_my_val", lambda: load_coco_json(VAL_JSON, VAL_PATH))
    MetadataCatalog.get("uavdt_my_val").set(thing_classes=CLASS_NAMES,  # 可以选择开启，但是不能显示中文，这里需要注意，中文的话最好关闭
                                           evaluator_type='coco',  # 指定评估方式
                                           json_file=VAL_JSON,
                                           image_root=VAL_PATH)
...
# 可选择使用此方法
def checkout_dataset_annotation(name="uavdt_my_val"):
